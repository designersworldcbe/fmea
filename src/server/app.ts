...
export const createApp = (ioArg?: Server) => {
  if (ioArg) io = ioArg;
  const app = express();
  ...
  // Comment Endpoints
  app.get("/api/fmeas/:id/comments", checkDB, async (req, res) => {
    try {
      const result = await pool.query("SELECT * FROM comments WHERE fmea_id = $1 ORDER BY created_at ASC", [req.params.id]);
      res.json(result.rows);
    } catch (e) {
      res.status(500).json({ error: (e as Error).message });
    }
  ...
  app.post("/api/fmeas/:id/comments", checkDB, async (req, res) => {
    const { user_id, user_name, text } = req.body;
    try {
      const result = await pool.query(`
        INSERT INTO comments (fmea_id, user_id, user_name, text)
        VALUES ($1, $2, $3, $4)
        RETURNING *
      `, [req.params.id, user_id, user_name, text]);
      
      // Broadcast to all clients
      if (io) io.emit("comment:added", result.rows[0]);
  ...
  app.get("/api/fmea-items/:id/assignments", checkDB, async (req, res) => {
    try {
      const result = await pool.query("SELECT * FROM assignments WHERE fmea_item_id = $1", [req.params.id]);
      res.json(result.rows);
    } catch (e) {
      res.status(500).json({ error: (e as Error).message });
    }
  ...
  app.post("/api/fmea-items/:id/assignments", checkDB, async (req, res) => {
    const { user_id, user_name, status } = req.body;
    try {
      const result = await pool.query(`
        INSERT INTO assignments (fmea_item_id, user_id, user_name, status)
        VALUES ($1, $2, $3, $4)
        RETURNING *
      `, [req.params.id, user_id, user_name, status]);
      
      // Broadcast to all clients
      if (io) io.emit("assignment:added", result.rows[0]);
